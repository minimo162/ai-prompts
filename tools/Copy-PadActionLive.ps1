...
param(
    ...
    [Parameter(Mandatory = $true)][string]$ExpectedActionName,
    [int]$ExpectedTotalActionCount = 1
)
...
if ($items.Count -ne $ExpectedTotalActionCount) { throw ('Unexpected total action count: ' + $items.Count) }
$selected = @($items | Where-Object { $_.Current.Name -ceq $ExpectedActionName })
if ($selected.Count -ne 1) { throw 'Expected captured action name was not unique' }
...
try {
    ...
    $targetItem.GetCurrentPattern([Windows.Automation.SelectionItemPattern]::Pattern).Select()
    $targetItem.SetFocus()
    ...
    if ($null -ne $copiedSequence -and (Get-AgentPadClipboardSequence) -eq $copiedSequence) {
        Restore-AgentPadClipboard $beforeClipboard
        $restored = $true
    }
}
[pscustomobject]@{
    ...
    action_name = $ExpectedActionName
    flow_name = $FlowName
} | ConvertTo-Json
