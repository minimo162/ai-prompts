...
$required = @(
    ...
    [ordered]@{ id='B-if-else'; area='B'; purpose='If／Else'; status='partial'; observed_action_ids=@('if-current-item-status-equals','if-current-file-contains-txt'); reason='If開始行を2設定採取し、For eachとの組合せはT03で実行確認済み。Else／Else ifの原文・実行は未確認。'; reference='catalog/index.json; catalog/actions/if/*.robin; catalog/evidence/normal-chat-t03-final-current-run.json' },
    [ordered]@{ id='B-loop-fixed-foreach-exit'; area='B'; purpose='回数指定ループ・For each・ループ脱出'; status='partial'; observed_action_ids=@('loop-one-to-three','foreach-items-variable'); reason='定数範囲LoopとFor eachの原文を採取し、For each＋If＋ファイル読取りのT03組合せを実行確認済み。ループ脱出・次のループは未確認。'; reference='catalog/index.json; catalog/evidence/normal-chat-t03-final-current-run.json' },
    ...
    [ordered]@{ id='C-datatable'; area='C'; purpose='データテーブル作成・行追加・行反復・セル参照'; status='partial'; observed_action_ids=@('datatable-create-empty','datatable-filter-status-equals-index2','variables-convert-datatable-csv-t04'); failed_action_ids=@('datatable-add-row-empty-failed'); reason='空のデータテーブル作成、Excel範囲→FilterDataTable条件抽出、DataTableのCSV文字列化は実測済み。0行0列への空行追加は設計エラーとして保存。列を持つテーブルの行追加・行反復・セル参照は未採取。'; reference='catalog/index.json; catalog/evidence/datatable-create-validation.json; catalog/evidence/t04-positive-flow-run-6.json; catalog/evidence/filter-t02-roundtrip2-run-2.json' },
    ...
    [ordered]@{ id='D-files-folders'; area='D'; purpose='ファイル取得・存在確認・フォルダー作成・テキスト読書き・コピー・移動・名前変更'; status='partial'; observed_action_ids=@('file-read-text-utf8','file-read-text-variable-path','file-write-text-utf8-overwrite','file-copy-single-no-overwrite','folder-get-files-txt-no-subfolders','folder-get-files-all-no-subfolders','file-write-t04-filtered-csv'); reason='UTF-8読取り・変数パス読取り・UTF-8書出し・単一ファイルコピー・フォルダー内ファイル取得2設定・T04中間CSV書出しは原文保存・別フロー貼付け・実行確認済み。存在確認、フォルダー作成、移動・名前変更は未確認。'; reference='catalog/index.json; catalog/evidence/text-write-validation.json; catalog/evidence/file-copy-validation.json; catalog/evidence/folder-get-files-rt-run-1.json; catalog/evidence/folder-get-files-all-rt-run-1.json; catalog/evidence/t03-partial-run-1.json; catalog/evidence/t04-positive-flow-run-6.json' },
    [ordered]@{ id='E-excel'; area='E'; purpose='Excel起動・セル／範囲読書き・シート・反復・別名保存・終了'; status='partial'; observed_action_ids=@('excel-launch-new-visible','excel-launch-open-readonly','excel-launch-t04-open-readonly','excel-launch-t04-filtered-csv-editable','excel-write-cell-a1-text','excel-read-cell-a1-typed','excel-read-range-a1-b2','excel-read-t04-range-a1-c6','excel-read-used-range','excel-save-as-xlsx','excel-save-t04-output-xlsx','excel-close-no-save','excel-close-t04-no-save'); reason='起動・セル／範囲読取り・別名保存・終了と、T04の読み取り専用範囲抽出→別CSV経由のxlsx保存を実測済み。シート選択・データ反復の独立例は未確認。'; reference='catalog/index.json; catalog/evidence/t04-positive-flow-run-6.json; catalog/evidence/t04-positive-flow-run-7.json' },
    ...
    [ordered]@{ id='G-ui-browser'; area='G'; purpose='起動／接続・文字入力・クリック・待機・文字取得・終了'; status='partial_blocked_runtime'; observed_action_ids=@('web-launch-t09-local','web-populate-t09-input','web-click-t09-submit','wait-t09-500ms','web-get-t09-result','web-close-t09'); reason='T09でローカルEdgeの3要素をUI要素ピッカーから捕捉し、6アクションを空フローへ貼付け・保存した。WebAutomation通し実行はブラウザー拡張経路のタイムアウトで未完了。未捕捉セレクターは推測していない。'; reference='docs/robin-action-catalog.md; copilot/knowledge/PAD-Robin-05-UI-Web.txt; catalog/evidence/t09-capture-source-20260910.json; catalog/evidence/t09-runtime-block-20260910.json' }
)
...
$result = [ordered]@{
    ...
    reconciliation_note = 'Existing index/evidence mappings are generated from the current catalog. This file is not a complete PAD feature reference or current-session execution claim.'
