...
$required = @(
    ...
    [ordered]@{ id='A-text-substring'; area='A'; purpose='文字列の切出し'; status='not_observed'; observed_action_ids=@(); reason='現在の実測カタログに該当アクションなし。表示名を推測していない。'; reference=$null },
    ...
    [ordered]@{ id='C-datatable'; area='C'; purpose='データテーブル作成・行追加・行反復・セル参照'; status='partial'; observed_action_ids=@('datatable-create-empty'); failed_action_ids=@('datatable-add-row-empty-failed'); reason='空のデータテーブル作成は今回の左パネル採取・再貼付け・実行まで確認済み。0行0列への空行追加は設計エラーとなったため失敗例として保存。列を持つテーブルの行追加・行反復・セル参照は未採取。'; reference='catalog/index.json; catalog/evidence/datatable-create-validation.json; catalog/evidence/datatable-add-row-empty-failed.robin' },
    [ordered]@{ id='C-csv-read-write'; area='C'; purpose='CSV読取り／書出し'; status='partial'; observed_action_ids=@('file-write-csv-pdf-table-headers','csv-read-utf8-system-default-no-header','csv-write-variable-reference'); reason='UTF-8 CSV読取り（ヘッダーを列名にしない）と、DataTable変数参照でのCSV書出しは今回の実測で確認済み。ヘッダーを列名にする設定・別区切り・引用符境界は未確認。'; reference='catalog/index.json; catalog/evidence/csv-read-validation.json' },
    [ordered]@{ id='D-files-folders'; area='D'; purpose='ファイル取得・存在確認・フォルダー作成・テキスト読書き・コピー・移動・名前変更'; status='partial'; observed_action_ids=@('file-read-text-utf8','file-copy-single-no-overwrite'); reason='UTF-8読取り、CSV読書き、単一ファイルのコピー（衝突時は何もしない）は実測済み。フォルダー内取得、存在確認、作成、テキスト書出し、移動・名前変更は未確認または別設定が必要。'; reference='catalog/index.json; catalog/evidence/file-copy-validation.json' },
