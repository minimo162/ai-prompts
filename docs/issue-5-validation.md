...
| PADアダプター | `tests/Test-Pad.ps1` **267 PASS**。UI/クリップボード境界を模擬し、全文一致・所有権・失敗時の旧フロー実行拒否・結果帰属を検査する。実機で観測したタイトル、保存・停止の直下Button、状態バーの構造と状態別の可視性、20種類の状態ID、エラー件数、貼り付け後の連続状態確認、保存中から保存済みへの遷移、実行中のStart不在、観測喪失中のキャンセルも本番関数で検証。実PAD貼り付けではない。 |
