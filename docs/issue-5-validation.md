...
静的・非ライブ検査はRaw 6、Package 3949/7/86、Catalog 94、DOM 898、CurrentBundle準備14、Issue状態23、JSON、`git diff --check`、非ライブ全件PASSを確認した（`catalog/evidence/current-package-static-check-20260911-p3d.json`）。新bundleのT01/T02/T04〜T08/T10、T01/T04/T10独立再試験、負例スイートまで、全文指示＋添付・無修正Robin・PAD実行・成果物照合を確認した。T03は厳密な拡張子／名前取得の原文不足でRobinなし、T09はUI要素登録とWebAutomation通し実行が外部依存でブロック中のため、Issue #5は`partial／OPEN`を維持する。
同一現行bundleでT03を通常M365 Copilotへ再送した。全文指示本文の逐語一致と`PAD-Robin-Knowledge-Bundle`／`agent-instructions`の2添付を確認したが、回答は厳密な拡張子・ファイル名取得、末尾`.txt`一致、完全な分岐構文を未確認として明示し、Robinコードブロックを返さなかった。このためT03は現行bundle受入に数えず、PAD貼付け・実行も行っていない。旧Final3のT03証跡はbundle hashが異なるため継承しない（`catalog/generated/normal-chat-current-bundle-p3b-t03-live-20260911/result.json`）。
同一現行bundleでT04を通常M365 Copilotへ送信し、9アクションRobin（空文字リテラル6引用符）を無修正保存した。専用PADフローへ貼付け・保存し、Designerの`9 アクション`表示、2回の実行、`ExcelData=6行3列`、`FilteredDataTable=3行3列`、`対象/A/10`・`対象/C/25`・`対象/D/5`の論理行、入力SHA不変を確認した。可視ListItemは6件に仮想化されたが、Designerステータスは9件で、手修正はない。出力xlsxのファイルSHAはExcelメタデータで変動するため固定値で判定していない。直接パーサーの日本語表示は未正規化として注記し、証跡はPADプレビューに限定する（`catalog/evidence/t04-current-p3b-live-acceptance-20260911.json`、`t04-current-p3b-pad-output-comparison-20260911.json`）。
同一現行bundleでT05を通常M365 Copilotへ送信し、4アクションRobinを無修正で専用PADフローへ貼付け・保存し、2回実行した。`ReadOnly=False`、B2への`T05-Changed`書込み、SaveAs、Closeを保持し、実行後に出力B2と入力A1の一致、元入力SHA不変を直接確認した（`catalog/evidence/t05-current-p3b-live-acceptance-20260911.json`、`t05-current-p3b-output-comparison-20260911.json`）。
同一現行bundleでT06を通常M365 Copilotへ送信し、4アクションRobinを無修正で専用PADフローへ貼付け・保存し、2回実行した。編集可能なWordで`OfficeCatalog`だけを`T06Replaced`へ置換して別名保存し、出力文書の置換結果・元入力SHA不変を直接確認した（`catalog/evidence/t06-current-p3b-live-acceptance-20260911.json`、`t06-current-p3b-output-comparison-20260911.json`）。
同一現行bundleでT07を通常M365 Copilotへ送信し、2アクションRobinを無修正で専用PADフローへ貼付け・保存し、2回実行した。PDFのページ2だけを抽出し、`PAGE_TOKEN_A2`を含み`PAGE_TOKEN_A1`を含まないUTF-8出力、入力PDF SHA不変を確認した（`catalog/evidence/t07-current-p3b-live-acceptance-20260911.json`、`t07-current-p3b-output-comparison-20260911.json`）。
同一現行bundleでT08を通常M365 Copilotへ送信し、欠損ファイルのエラー処理Robinを無修正で専用PADフローへ貼付け・保存し、2回実行した。`ErrorHandledDefault=true` と `LastError` の「ファイルが見つかりません」内容を両回確認し、後続の書込み・公開処理は含めなかった。名前付き`FileNotFound`一致は未確認として保持した（`catalog/evidence/t08-current-p3b-live-acceptance-20260911.json`、`t08-current-p3b-output-comparison-20260911.json`）。
同一現行bundleでT10を通常M365 Copilotへ現行T10コンテキスト添付とともに送信し、16命令Robinを無修正で専用PADフローへ貼付け・保存し、2回実行した。Excelの書込み値とExcel SaveAs先だけが変更され、Word／PowerPointの`CopilotOffice 246`と既存処理は保持された。Excel A1、Word本文、PowerPointスライドの成果物を直接照合し、可視ListItem 6件に対してDesigner 16アクションを確認した（`catalog/evidence/t10-current-p3b-live-acceptance-20260911.json`、`t10-current-p3b-output-comparison-20260911.json`）。
