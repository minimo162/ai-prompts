...
| PADアダプター | `tests/Test-Pad.ps1` **258 PASS**。UI/クリップボード境界を模擬し、全文一致・所有権・失敗時の旧フロー実行拒否・結果帰属を検査する。実機で観測したタイトル、保存・停止の直下Button、状態バーの構造と状態別の可視性、20種類の状態ID、エラー件数、貼り付け後の連続状態確認、保存中から保存済みへの遷移も本番関数で検証。実PAD貼り付けではない。 |
...
| 2: AIなしのA/B差し替え | 未合格・開始前の失敗を確認 | 実デザイナーで貼り付け・保存・実行・開始終了観測、失敗で旧フローを走らせない |
