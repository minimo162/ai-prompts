...
Run1のxlsxスナップショットをRun2前に取得できなかったため、独立T04は `PASS_CURRENT_20260914E_INDEPENDENT_T04_GENERATION_PAD_TWO_RUNS_PARTIAL`／`candidate_partial` として追跡し、受入済み一覧へ追加していない。3回目のRunは行わず、元T04 attempt3の応答由来literalバックスラッシュ不受入を上書き・昇格していない。証跡は `catalog/generated/normal-chat-20260914e-t04-independent-live/` と `catalog/evidence/t04-independent-current-*20260914e*`。残る独立T10、P3正例、負例v2、A〜G監査はOPEN / partialのまま。
