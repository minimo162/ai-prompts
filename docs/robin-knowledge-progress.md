...
リスト項目取得は、表示された「リストから項目を削除」を作成→追加→削除の最小probeで確認したが、削除後に項目値を返す出力変数がなく、取得構文の代用にはならなかった。現行の専用フローで左パネルの読取りインベントリも試みたが、仮想化スクロールの終端が安定せず、401ノード以上を観測した時点で停止した。推測で追加せず、未確認として保持する（`catalog/evidence/p3-list-remove-probe-20260910.json`、`catalog/evidence/p3-list-get-inventory-boundary-20260911.json`）。
