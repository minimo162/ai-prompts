...
次の必須確認は残っています: ポリシー無効の明示案内、送信確認/生成中/失敗種別の一元的な接続契約、匿名fixture・対応版の索引、1/50/100件と長文の実M365ベンチ、会話分離の比較、表示倍率・前面変更・スリープ・認証切れ、社内PC/別利用者の対応表。少数1回の成功を全件数・全環境の保証にしません。
