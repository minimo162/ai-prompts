...
次の必須確認は残っています: 失敗種別の追加分離、匿名fixture・対応版の索引、1/50/100件と長文の実M365ベンチ、会話分離の比較、表示倍率・前面変更・スリープ・認証切れ、社内PC/別利用者の対応表。現在の実環境の読取り診断は`CDP_UNAVAILABLE`（専用Copilotタブ未確認）で、送信を行っていません。少数1回の成功を全件数・全環境の保証にしません。
