...
const assistants=nodes.map((e,i)=>({key:e.getAttribute('data-message-id')||e.id||String(i),text:assistantText(e),collapsed:!!e.querySelector('button[aria-expanded="false"],[data-state="collapsed"]')}));
...
function Invoke-AgentCopilot {
    ...
    try {
        ...
        $wirePrompt=$Prompt.Replace("`r`n","`n")+"`n`n応答全体は厳密に 2 行にしてください。第1行: 指定された単一の JSON オブジェクト。トップレベルの request_id は `"$RequestId`" とし、文字列中の改行は JSON のエスケープで表してください。第2行: AGENT_END_$RequestId だけを出力してください。ほかの文字、Markdown、コードフェンス、前後の説明は一切付けないでください。"
        $inputStarted=$false
