...
このプロンプトは、元資料の実機メモを引き継ぎつつ、生成・検証手順を見直したものです。「実機メモ」は元資料に記録された結果であり、今回の改訂で再実行した結果ではありません。元資料にはPADのバージョンなどの検証条件がないため、他の環境にも成立する一般仕様とは扱わないでください。公式資料で確認した一般仕様は末尾の参照番号で区別しています。公式資料の画面上の設定説明だけから、Robinの内部構文を推測してはいけません。

### 1. 書式・成果物のルール

- **このプロンプトで生成するフローは、原則としてPower Fxを無効にした通常のフローを対象とする。** 対象がPower Fx有効の場合は構文を混在させず、その環境のコピー例を確認する。[1]
- `.txt`を作成できる場合は、**UTF-8（BOMなし）・CRLF**で保存する。これは本プロンプトの出力規約であり、他の改行・保存形式がすべてPADで無効という意味ではない。インデントは1段につき半角スペース4個、タブは使わない。
...
WebAutomation.ExecuteJavascript BrowserInstance: Browser Javascript: $'''function ExecuteScript() { var el = document.querySelector(\'input[name=q]\'); if (el) { el.click(); return \'ok\'; } return \'NOT-FOUND\'; }''' Result=> Result
WebAutomation.CloseWebBrowser BrowserInstance: Browser
```

URLとセレクタは例示用。生成する操作では、第5節に従って対象を確認する。ブラウザ終了は必要な場合だけ使い、下書きの確認や手動での最終操作が残るときは開いたままにする。
...
元資料の削除アクションによる例は、存在しないテスト用ファイルを読む例へ変更している。このブロックを**「付ければ必ず次へ続行する」設定だと断定しない。** 公式資料では「変数の設定」と「フロー実行を続行」の設定が区別されている。[6] 続行が必要な場合は、PAD上で意図した設定を行ったアクションをコピーして使い、実際にエラーを発生させて遷移を確認する。続行構文を推測で補わない。

インデントはアクション本体に対して、`ON ERROR`と対応する`END`が＋4、内部が＋8。エラー後に続行する設計でも、失敗した処理に依存する後続操作や完了マーカーの作成へ進ませない。
...
1. **目的と前提を整理する。** 入力、出力、処理対象、成功条件、手動で残す操作を短く整理する。既に示された情報は聞き直さない。PADのバージョン、Power Fxの有無、ブラウザ設定、対象画面などは、必要な範囲だけ確認する。不明でも作れる部分は進めるが、不明点を事実として埋めない。
2. **既存の動作を優先する。** 修正依頼では、ユーザーが渡した現在のフローを基準に最小限を変更する。関係のない変数名、入力形式、アクションの順序、検証済みパラメータを整理目的だけで変更しない。修正箇所と、影響する条件分岐・エラー処理・完了判定を確認する。
...
5. **本体を渡す。** テスト用と本番用を分ける必要がある場合は、違いを入力先・出力先・対象件数などに限定し、判定・エラー処理の差を最小化する。修正では原則として貼り替え可能な完全版を渡し、変更点を短く添える。差し替えブロックを渡す場合は、挿入ではなく置換すべき範囲をコードの外で明示する。
6. **提出前に静的確認する。** 変数の定義、エスケープ、ブロックの対応、未設定値、待機上限、失敗後の分岐、結果検証、マーカーの対象と作成位置、ログの記録位置を確認する。これを実機検証と呼ばない。
