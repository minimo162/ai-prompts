...
`agent-instructions.txt` の実測は UTF-8 BOMなし、Unicodeスカラー数3,401、UTF-16コード単位数3,401（8,708バイト）です。8,000文字上限以内です。通常チャットの本文へ全文を入力した実測と、送信後の欠落有無を別証跡へ保存します。
...
2026-09-10時点では、配布物の骨格、86件の観測バリアント、既存証拠、PAD左パネル観測、日時・DataTable・CSV・FilterDataTable・ファイルコピー・サブテキスト・テキスト書出し・テキスト変数書込み・For each・If・Excel/Word編集可能起動・フォルダー取得・ファイル変数読取り・PDFページ2単独抽出・T04 Excel条件抽出→別xlsx・T09ローカルUI要素捕捉の追加実測を整理しました。7ファイルの逐次添付が不安定なため、元ファイル名・区切り・SHA-256を保持したフォールバック結合版を使用します。現作業版結合版のSHA-256は `0d09c90f8f95bd235a3d05cabd1bf76a445be3037eaa3bd5a46bbfee658bf10e` です。通常のM365 CopilotチャットでGPT 5.6 Think Deeperを選択し、現作業版と合成Excelを添付したT04拒否境界はbundle `2a0267d4…` で確認済みです。その後、T04正例をPADで2回実測し、条件一致3行を別xlsxへ保存できることを確認しました。現作業版bundleでのT04通常チャット再生成、T01/T04/T10独立再試験、T09 WebAutomation実行は未完了です。T09はUI要素3件の実捕捉・6アクションの空フロー貼付け・保存まで確認し、実行はブラウザー拡張経路のタイムアウトで停止しました（`catalog/evidence/t09-runtime-block-20260910.json`）。旧結合版でのT01/T02/T03/T05/T06/T07/T08/T10実行記録は履歴として保持しますが、現作業版の最終受入には再試験が必要です。ナレッジ固有事前質問は前版bundle（2a版）で別チャットへ供給し、回答原文を照合済みです。正例追加後の現作業版では再確認が必要です。Agent Builder／Copilot Studioの登録・権限確認は将来の別環境の確認事項であり、今回の完了条件には含めません。未確認の項目を完成扱いしません。
