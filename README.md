...
画面は「やりたいこと」と「作業対象」から依頼する構成です。PAD左パネルから採取した[Robinカタログ](catalog/README.md)を根拠に、プロンプトと検証器を広げています。現在の追加対応はリスト作成・文字列項目追加、通常/正規表現のテキスト置換です。未定義変数や分岐後の不確かな型は実行前に拒否します。採取例からCopilotが生成した1ケースの手動通し検証と、アプリの自動Run経路の検証は区別しています。後者の新操作を含む実機通しは未完了です。
