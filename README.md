...
2. PADで、Power Fxを無効にした空の「業務エージェント専用」フローを作成し、Mainデザイナーを開きます。既存業務フローを指定しないでください。アプリは専用フローに自分で反映したアクションだけを次回以降置き換えます。
...
テスト結果を実機合格や他PC対応の証明として使わないでください。PADのセレクター候補はインストール済み2.71リソースから得たもので、実際のデザイナーでの一意性・貼り付け・保存・実行確認が必要です。
