...
有限Loopの`NEXT LOOP`も別probeでLoopIndex=4を2回確認していますが、入れ子とloop内副作用は未確認です。
エラー処理の`BLOCK / ON BLOCK ERROR / THROW ERROR / END`骨格も別probeで確認していますが、エラー発生子アクションとサブフローは未確認です。
