...
if ($audit.PSObject.Properties.Name -contains 'p4_current_bundle') {
    ...
    if ($audit.p4_current_bundle.status -ceq 'PARTIAL_NEW_BUNDLE_LIVE_ACCEPTANCE_PENDING') {
        ...
        Check (($audit.p4_current_bundle.t08.status -ceq 'NOT_RUN_NEW_BUNDLE') -or ($audit.p4_current_bundle.t08.status -ceq 'PASS_CURRENT_BUNDLE_EXPECTED_ERROR')) 'new audit keeps T08 state explicit'
