...
await page.locator('#artifacts details > summary').click();a.equal(await page.locator('#artifacts pre').innerText(),'preview text');checks++;
...
// A different history entry with copied view metadata must not reuse the old button closure.
const otherId=randomUUID().replaceAll('-',''),otherDir=path.join(home,'data/jobs',otherId);fs.mkdirSync(otherDir);const other=JSON.parse(fs.readFileSync(path.join(home,'data/jobs',fixture.job_id,'job.json'),'utf8'));other.job_id=otherId;other.goal='copied metadata fixture';fs.writeFileSync(path.join(otherDir,'job.json'),JSON.stringify(other));
await page.locator('#job-history option[value="'+otherId+'"]').waitFor({state:'attached'});await page.locator('#job-history').selectOption(otherId);await page.waitForFunction(id=>document.querySelector('#job-reference').textContent.includes(id),otherId);await page.locator('#artifacts button').click();await page.waitForFunction(()=>document.querySelector('#action-message').textContent.includes('INVALID_PATH'));a.equal(requests.at(-1).job_id,otherId);checks++;
