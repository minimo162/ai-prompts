...
現在は変数・テキスト・ループ・ファイル読取り・Office・PDF操作の34種類・51設定例です。初期の専用フローは`RobinCatalog_20260907`、Officeの採取先はユーザーが用意した`test`です。コピー前のクリップボードはメモリーに保持して復元し、その内容をファイルへ記録していません。
