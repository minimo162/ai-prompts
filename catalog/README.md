...
> 2026-09-13 追跡: T03は固定カウンター要件を依頼本文へ明示した候補限定の再生成を無修正PADへ貼付け、2回実行（2 txt／2 other）まで確認しました。T04は生成回答の`=>`前バックスラッシュをDOMで対照し、原記録を維持しています。候補の指示WAIT 1（SHA `6ad6f742…`）と配布正本のWAIT 500（SHA `b4a3c24f…`）は別版で、候補証跡を正本へ昇格したとは扱いません。現在の判定はOPEN / partialです（`evidence/current-package-status-20260913.json`）。
